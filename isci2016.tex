...
\title{Biologically Plausible Training on \\ Rate-based Spiking Autoencoders}
...
\begin{figure}
	\centering
		\includegraphics[width=0.4\textwidth]{pic/RBM.pdf}
...
\begin{equation}
L=\frac{1}{2}\sum_{k=1}^{K}\sum_{i=1}^{N}(x_{ki}-x^{'}_{ki})^{2}
\end{equation}
where k indicates the index of the training data $\textbf{x}_{k}$, and $x_{i}$ is the $i$th dimension of a data vector $\textbf{x}$.
...
Applying stochastic gradient descent (SGD), the true gradient of $\Delta W$ is approximated by a gradient at a single example with iterations:
\begin{align*}
%\begin{spilt}
E_k &= \frac{1}{2}\sum_{i=1}^{N}(x_{ki}-x^{'}_{ki})^{2}, \textrm{ thus} \\
\Delta w_i &\propto -\frac{\partial E_k}{\partial w_i}=-\eta \frac{\partial E_k}{\partial w_i}.
%\end{split}
\end{align*}


\section{Results}
\subsection{Classify MNIST Digits}
