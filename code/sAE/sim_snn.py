...
def update_up(layer, input_spikes, neuron_conf):
    ...
    index = layer['hid_v']>=(neuron_conf['v_thresh'])# + np.random.normal(0, 0.2, layer['hid_v'].shape))
...
def update_down(layer, input_spikes, neuron_conf):
    ...
    # check threshold to generate spikes
    index = layer['vis_v']>=(neuron_conf['v_thresh'])# + np.random.normal(0, 0.2, layer['vis_v'].shape))
...
def greedy2(layer, input_spikes, teach_spikes, neuron_conf, train_conf):
    # get h
    layer = update_up(layer, input_spikes, neuron_conf)
    ...
    for i in range(train_conf['tau_stdp']):
        w_plus = np.einsum('ij,ik->ijk', teach_spikes, train_conf['delta_w'][i]*layer['hid_last_spikes'][i]).mean(axis=0) #teach_spikes
        layer['w_up'] += w_plus
...
def greedy(layer, input_spikes, neuron_conf, train_conf):
    # get h
    layer = update_up(layer, input_spikes, neuron_conf)
    layer['hid_last_spikes'].pop(-1)
    layer['hid_last_spikes'].insert(0, layer['hid_spikes'].astype('int')) # newly generated spikes, the time diff = 0
    
    # w+ when  (input_spikes) arrives
    for i in range(train_conf['tau_stdp']):
        w_plus = np.einsum('ij,ik->ijk', input_spikes, train_conf['delta_w'][i]*layer['hid_last_spikes'][i]).mean(axis=0) #teach_spikes
        layer['w_up'] += w_plus
...
def train_layer2(dbnet, curr_l, neuron_conf, train_conf, train_x, train_y=[], record=False):
    ...
    for offset in range(b_size, train_x.shape[0]+1, b_size):
        ...
        for dt in range(train_conf['duration']): #train_conf['duration']
            ...
            layer = greedy2(layer, input_spikes,  teach_spikes, neuron_conf, train_conf)
            if record:
    ...
    for offset in range(b_size, train_x.shape[0]+1, b_size):
        ...
        for dt in range(train_conf['duration']): #train_conf['duration']
            ...
            if layer['class'] == 'top':
                ...
                label_spikes = spike_gens(labels, rate=rate, method=method)
                input_spikes = np.append(input_spikes,label_spikes,axis=1)
            layer = greedy(layer, input_spikes, neuron_conf, train_conf)
            if record:
                w_list.append(np.copy(layer['w_up']))
...
def dbn_test(dbnet, neuron_conf, test_x, test_conf,  model='recon', test_y=[]):
    sumeveryxrows = lambda myarray, x: myarray.reshape((myarray.shape[0] / x, x)).sum(0)
    rate = test_conf['rate']
    method = test_conf['method']
    b_size = test_conf['batch_size']
    
    for dt in range(test_conf['duration']):
        input_spikes = spike_gens(test_x, rate=rate, method=method)
        for l in range(len(dbnet)-1):
            dbnet[l] = update_up(dbnet[l], input_spikes, neuron_conf)
            input_spikes = dbnet[l]['hid_spikes']

        # top layer reconstruciton
        layer = dbnet[-1]
        if layer['class']=='top':
            layer = update_top(layer, input_spikes, neuron_conf, b_size)
            input_spikes =  layer['vis_spikes'][:, :layer['vis_size'][0]]
            
        else:
            layer = update_up(layer, input_spikes, neuron_conf)
            input_spikes = layer['hid_spikes']
            layer = update_down(layer, input_spikes, neuron_conf)
            input_spikes = layer['vis_spikes']
        
        if model=='label': # aim on classification
            count = layer['vis_sum_spikes'][:, layer['vis_size'][0]:]
            count = np.array([sumeveryxrows(c,10) for c in count])
            predict = np.argmax(count, axis=1)
            ca = np.sum(predict==test_y)*100./b_size
            if np.mod(dt+1, test_conf['duration'])==0:
                print 'CA with %d ms test: %.1f'%(dt+1, ca)
            test_conf['result'].append(ca)
        elif model=='recon': # aim on reconstruction
            for l in range(len(dbnet)-1, 1, -1):
                dbnet[l-1] = update_down(dbnet[l-1], input_spikes, neuron_conf)
                input_spikes = dbnet[l-1]['vis_spikes']
                
    if model=='label':
        return test_conf['result']
    else:
        return dbnet
