...
def init_para(vis_num, hid_num, eta):
    ...
    #w = np.random.uniform(0.,1., vis_num*hid_num)
...
def update_batch_cd1(para, data_v):
    ...
    for i in range(max_bsize):
        ...
        #v0h0-v1h1 =>  h0(v0-v1)
        neg_delta_w += matu.matrix_times(gibbs_v[i], data_h[i])
...
def RBM_train(para, batch_size, train_data):
...
def RBM_train2(para, batch_size, train_data, decay=0):
    train_num = train_data.shape[0]
    w_list = list()
    #for iteration in range(epoc):
    for k in range(0,train_num,batch_size):
        ...
        data_v = train_data[k:k+max_bsize]
        para = update_batch_cd1(para, data_v)
        w_list.append(np.copy(para['w']))
        if np.mod(k*batch_size,1000)==0 and decay==1:
            para['eta'] *= 0.5
    return para, w_list

def greedy_train(dbnet, curr_l=0):
    ...
    for i in range(curr_l):   #len(dbnet['layer']) #bottom up
        #dbnet['layer'][i] = RBM_train(dbnet['layer'][i], dbnet['epoc'], batch_size, train_data)
        train_data = ReLU(train_data, dbnet['layer'][i]['w'])
    if curr_l == len(dbnet['layer']):
        train_data = np.append(train_data, train_label, axis=1)
        dbnet['top'] = RBM_train(dbnet['top'], batch_size, train_data)
    else:
        dbnet['layer'][curr_l] = RBM_train(dbnet['layer'][curr_l], batch_size, train_data)
    return dbnet
